...
* What do the various formats look like? Zoom in to view the alignment information.

* Let's change the name of the track generated by the wig file, since the current one is worse than useless. 

* Do you observe any correlation with where the peaks are patterns of alignment in the Nanog and Input tracks?

* Suggested genes and surrounding areas to look at:
...
* When you zoom out, do you see the blue mass on the wig track closer to the beginning of the chromosome? Any guesses as to what could be causing that "negative" coverage?

* Collapse the alignment data to fit a little more snugly in the track.

* To compare what our consolidated peaks look like relative to the peaks generated by Encode for the same dataset (H1-hESC Nanog IP) let's bring in those peaks from ENCODE.

* Let's also bring in ()broad) peaks for H3K4me1 (a mark for active/poised enhancers). How do you think the peaks for the histone modification compare to the Nanog peaks?
***
